...
fn explore_trail(
    ...
    explore_trail(level, input, (position.0, position.1 + 1), size)
        + explore_trail(level, input, (position.0 + 1, position.1), size)
        + explore_trail(level, input, (position.0, position.1 - 1), size)
        + explore_trail(level, input, (position.0 - 1, position.1), size)
}
...
pub fn main() {
    ...
    let total: i32 = find_trail_heads(&input)
        .iter()
        .map(|value| explore_trail(-1, &input, *value, size))
        .sum();
